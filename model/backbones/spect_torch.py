...
def drop_path(x, drop_prob: float = 0., training: bool = False):
    ...
    shape = (x.shape[0],) + (1,) * (x.ndim - 1)  # 使用diff-dim张量，而不仅仅是2D卷积
...
class HybridEmbed(nn.Module):
    ...
    """
    def __init__(self, backbone, img_size=224, feature_size=None, in_chans=3, embed_dim=384):
        super().__init__()
...
class PatchEmbed_overlap(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, stride_size=20, in_chans=3, embed_dim=384):
        super().__init__()
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
...
class SpectFormer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        super().__init__()
        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dim  # # 为了与其他模型保持一致，设置num_features等于embed_dim
        self.local_feature = local_feature
        if hybrid_backbone is not None:
            self.patch_embed = HybridEmbed(
                hybrid_backbone, img_size=img_size, in_chans=in_chans, embed_dim=embed_dim)
        ...
        # for i in range(depth):
        #     if i < alpha:
        #         layer = Block_spect(dim=embed_dim, mlp_ratio=mlp_ratio, drop=drop_rate, drop_path=dpr[i],
        ...
        self.apply(self._init_weights)
        print('spectformer architecture ready')
    ...
    #加载模型参数的函数，从model_path加载参数字典，并将参数应用于当前模型对象
    def load_param(self, model_path):
        param_dict = torch.load(model_path, map_location='cpu')
        if 'model' in param_dict:
            param_dict = param_dict['model']
        if 'state_dict' in param_dict:
            param_dict = param_dict['state_dict']
        for k, v in param_dict.items():
            k = k.replace('base.', '').replace('module.', '')  # 移除 'base.' 和 'module.' 前缀
            if 'patch_embed.proj.weight' in k and len(v.shape) < 4:
                # 对于在卷积基础上进行路径化之前训练的旧模型
                O, I, H, W = self.patch_embed.proj.weight.shape
                v = v.reshape(O, -1, H, W)
            elif k == 'pos_embed' and v.shape != self.pos_embed.shape:
                # 在使用预训练权重的不同尺寸的模型上调整位置嵌入
                if 'distilled' in model_path:
                    print('distill need to choose right cls token in the pth')
                    v = torch.cat([v[:, 0:1], v[:, 2:]], dim=1)
...
def spectformer_xs_patch16_224(img_size=(256, 256), stride_size=16, drop_path_rate=0.1, drop_rate=0.0, attn_drop_rate=0.0, camera=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
    model = SpectFormer(
...
def spectformer_Base_patch16_224(img_size=(256, 256), stride_size=16, drop_path_rate=0.1, drop_rate=0.0, attn_drop_rate=0.0, camera=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
    kwargs.setdefault('qk_scale', 768 ** -0.5)
    model = SpectFormer(
