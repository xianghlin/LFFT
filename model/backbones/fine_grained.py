...
class SpectralGatingNetwork(nn.Module):
    ...
    def forward(self, x, spatial_size=None):
        ...
        x = x * weight
        x = torch.fft.irfft2(x, s=(a, b), dim=(1, 2))
...
class Spect_Block(nn.Module):

    def __init__(self, dim, mlp_ratio=4., drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, h=64,w=64):
        super().__init__()
    ...
    def forward(self, x):
        # h = self.norm1(x)
        # h,weights,contribution = self.filter(h)
        #
        #
        # output = self.drop_path(self.mlp(self.norm2(h)))

        output = self.drop_path(self.mlp(self.norm2(self.filter(self.norm1(x)))))
        # x = x[:, :256, :] + output
        x = x+ output
        return x
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        #卷积提取特征，其内核大小和跨度长度等于补丁大小。这相当于将图像分割成patch，并对每个patch进行线性变换。
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)

    #x是输入图像的形状
    def forward(self, x):
        #B:batch_size C:channels H:hight W:width
        B, C, H, W = x.shape
...
class fina_grained(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.spect_blocks = nn.ModuleList()
        for i in range(4):
            layer = Spect_Block(dim=embed_dim, mlp_ratio=mlp_ratio, drop=drop_rate, drop_path=dpr[i],
                                norm_layer=norm_layer, h=h, w=w)
            self.spect_blocks.append(layer)
        self.spect_layer = Spect_Block(dim=embed_dim, mlp_ratio=mlp_ratio, drop=drop_rate,
                                 norm_layer=norm_layer, h=h, w=w)
        ...
        # # 创建一个单独的Transformer块，用于处理选定的部分
        # self.part_layer = Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
        #         drop=drop_rate, attn_drop=attn_drop_rate, norm_layer=norm_layer)
        self.part_layer = copy.deepcopy(last_block)
        # 创建LayerNorm层，用于归一化选定部分的输出
        ...
        # Classifier head
        self.fc = nn.Linear(embed_dim, 576) if num_classes > 0 else nn.Identity()
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks:
            if i==7:
                ...
                # part_states, part_weights = blk(concat)
        ...
        #     selected_num, selected_inx = self.part_select(weights, contribution)
        #     B = selected_inx.shape[0]
        #     for i in range(B):
        #         tokens[i].extend(x[i, selected_inx[i, :self.num_token]])
        #     attn_weights.append(weights)
        ...
        # concat = torch.cat((x_copy[:, 0].unsqueeze(1), tokens), dim=1)
        # # print('concat shape', concat.shape)
        # ff_states, ff_weights, ff_contri = self.part_layer(concat)
        # encoded = self.part_norm(ff_states)

        print(part_encoded.shape,x.shape)
        return part_encoded[:, 0],x[:,0]
    ...
    def forward(self, x, cam_label=None, view_label=None):
        part,glo= self.forward_features(x)
        return part,glo
    ...
    def load_param(self, model_path):
        ...
        for k, v in param_dict.items():
            ...
            if 'patch_embed.proj.weight' in k and len(v.shape) < 4:
                ...
                v = resize_pos_embed(v, self.pos_embed)
            try:
...
#posemb为原始的位置嵌入张量，posemb_new为目标大小的位置嵌入张量
...
def vit_base_patch16_224_TransReID(img_size=(256, 256), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5, **kwargs):
    model = fina_grained(
        img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=768, depth=8, num_heads=12, mlp_ratio=4, qkv_bias=True,\
...
def deit_small_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, drop_rate=0.0, attn_drop_rate=0.0, camera=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
    model = fina_grained(
        img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=384, depth=8, num_heads=6, mlp_ratio=4, qkv_bias=True,
