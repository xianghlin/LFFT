        ...
        if cfg.DATASETS.NAMES == 'veri':
            for trial in range(10):
                ...
                rank_1, rank5 = do_inference(cfg,
                     ...
                     val_loader,
                     num_query)
                # rank_1, rank5,ranked_results = do_inference(cfg, model, val_loader, num_query)

                # for i in range(min(10, len(ranked_results))):
                #     rank_info = ranked_results[i]
                #     image_path = rank_info['image_path']
                #     rank = rank_info['rank']
                #     score = rank_info['score']
                #
                #     image = cv2.imread(image_path)
                #     cv2.imshow(f"Rank {rank} (Score: {score:.2f})", image)
                #     cv2.waitKey(0)
